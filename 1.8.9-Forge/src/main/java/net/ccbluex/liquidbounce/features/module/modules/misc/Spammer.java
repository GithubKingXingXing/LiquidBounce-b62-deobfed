...
public class Spammer extends Module
{
                ...
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("message")) {
                        if (args.length > 2) {
                            ...
                            Spammer$3.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                            return;
                        }
    ...
    public void onUpdate(final UpdateEvent event) {
        if (this.msTimer.hasTimePassed(this.delay)) {
            Spammer.mc.thePlayer.sendChatMessage(this.customValue.asBoolean() ? this.replace((String)this.messageValue.asObject()) : (this.messageValue.asObject() + " >" + RandomUtils.randomString(5 + RandomUtils.getRandom().nextInt(5)) + "<"));
            this.msTimer.reset();
