...
public class Speed extends Module
{
                ...
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("mode")) {
                        if (args.length > 2 && Speed.this.modeValue.contains(args[2])) {
                            ...
                            Speed$2.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                            return;
                        }
    ...
    public void onUpdate(final UpdateEvent event) {
        if (Speed.mc.thePlayer.isSneaking()) {
            return;
        }
    ...
    public void onMotion(final MotionEvent event) {
        if (Speed.mc.thePlayer.isSneaking() || event.getEventState() != EventState.PRE) {
            return;
        }
        if (MovementUtils.isMoving()) {
            Speed.mc.thePlayer.setSprinting(true);
