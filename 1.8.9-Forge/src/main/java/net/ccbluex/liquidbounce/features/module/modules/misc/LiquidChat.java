...
public class LiquidChat extends Module
{
        ...
        LiquidBounce.CLIENT.commandManager.registerCommand(new Command("LiquidChat", new String[] { "lc" }) {
            ...
            public void execute(final String[] args) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    if (LiquidChat$1.mc.thePlayer == null) {
                        return;
                    }
        ...
        LiquidBounce.CLIENT.commandManager.registerCommand(new Command("liquidchatpn", new String[] { "lcpn" }) {
            ...
            public void execute(final String[] args) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    if (LiquidChat$2.mc.thePlayer == null) {
                        return;
                    }
                    ChatUtils.displayChatMessage("§c§lError: §7" + e.getClass().getName() + ": " + e.getMessage());
                }
    ...
    public void onStarted() {
        ...
        this.connect(LiquidChat.mc.getSession().getUsername());
    }
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if (this.socket != null && this.username != null && !this.username.equals(LiquidChat.mc.getSession().getUsername())) {
            this.disconnect();
            this.connect(LiquidChat.mc.getSession().getUsername());
        }
    }
    ...
    private void connect(final String username) {
        try {
            ...
            this.sendPacket(new AuthenticatePacket(username, LiquidChat.mc.getSession().getProfile().getId(), StringUtils.hashString(this.getHWID().replace(" ", "")), LiquidChat.mc.getSession().getToken().equals("-") || LiquidChat.mc.getSession().getToken().isEmpty() || LiquidChat.mc.getSession().getSessionType().name().equals("LEGACY") || LiquidChat.mc.getSession().getToken().equals("0")));
        }
        catch (SocketException e) {
            if (LiquidChat.mc.thePlayer == null) {
                return;
            }
            ChatUtils.displayChatMessage("§c§lError: §7" + e.getClass().getName() + ": " + e.getMessage());
            this.setState(false);
