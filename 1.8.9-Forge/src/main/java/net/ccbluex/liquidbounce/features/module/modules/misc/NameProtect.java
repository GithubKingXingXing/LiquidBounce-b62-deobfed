...
public class NameProtect extends Module
{
        ...
        LiquidBounce.CLIENT.commandManager.registerCommand(new Command("nameprotect", null) {
            ...
            public void execute(final String[] args) {
                ...
                if (args.length > 2) {
                    ...
                    NameProtect$1.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                    return;
                }
    ...
    public void onText(final TextEvent event) {
        if (NameProtect.mc.thePlayer == null || event.getText().contains("§8[§9§lLiquidBounce§8] §3")) {
            return;
        }
        ...
        event.setText(StringUtils.replace(event.getText(), NameProtect.mc.thePlayer.getName(), ChatColor.translateAlternateColorCodes(this.fakeNameValue.asString()) + "§f"));
        if (this.allPlayersValue.asBoolean()) {
            for (final NetworkPlayerInfo playerInfo : NameProtect.mc.getNetHandler().getPlayerInfoMap()) {
