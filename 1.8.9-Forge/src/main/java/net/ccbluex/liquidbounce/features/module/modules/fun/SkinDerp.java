    ...
    public void onUpdate(final UpdateEvent event) {
        if (this.timer.hasTimePassed(this.delayValue.asInteger())) {
            ...
            if (this.hatValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.HAT, random.nextBoolean());
            }
            if (this.jacketValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.JACKET, random.nextBoolean());
            }
            if (this.leftPantsValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.LEFT_PANTS_LEG, random.nextBoolean());
            }
            if (this.rightPantsValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.RIGHT_PANTS_LEG, random.nextBoolean());
            }
            if (this.leftSleeveValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.LEFT_SLEEVE, random.nextBoolean());
            }
            if (this.rightSleeveValue.asBoolean()) {
                SkinDerp.mc.gameSettings.setModelPartEnabled(EnumPlayerModelParts.RIGHT_SLEEVE, random.nextBoolean());
            }
            this.timer.reset();
        }
    }
