...
public class TabGUI extends Element
{
    ...
    private void drawGui(final int posX, final int posY) {
        ...
        for (int i = 0; i < this.tabs.size(); ++i) {
            ...
            this.fontRenderer.drawString(tabName, (this.getFacing().getHorizontal() == Facing.Horizontal.RIGHT) ? (posX + this.width.asInteger() - this.fontRenderer.getStringWidth(tabName) - this.tabs.get(i).textFade - 3.0f) : (posX + this.tabs.get(i).textFade + 2.0f), (float)(yOff + this.textPositionY.asInteger()), (this.selectedTab == i) ? 16777215 : new Color(210, 210, 210).getRGB(), this.textShadow.asBoolean());
            if (this.arrowsValue.asBoolean()) {
                if (this.getFacing().getHorizontal() == Facing.Horizontal.RIGHT) {
                    this.fontRenderer.drawString((!this.mainMenu && this.selectedTab == i) ? ">" : "<", (float)(posX + 3), (float)(yOff + 2), 16777215, this.textShadow.asBoolean());
                }
                else {
                    this.fontRenderer.drawString((!this.mainMenu && this.selectedTab == i) ? "<" : ">", (float)(posX + this.width.asInteger() - 8), (float)(yOff + 2), 16777215, this.textShadow.asBoolean());
                }
            }
    ...
    private class Tab
    {
        ...
        private void drawTab(int x, int y, final int color, final int backgroundColor, final int borderColor, final int borderStrength, final boolean upperCase) {
            ...
            for (final Module module : this.modules) {
                if (TabGUI.this.fontRenderer.getStringWidth(upperCase ? module.getName().toUpperCase() : module.getName()) + 4 > maxWidth) {
                    maxWidth = (int)(TabGUI.this.fontRenderer.getStringWidth(upperCase ? module.getName().toUpperCase() : module.getName()) + 7.0f);
                }
            }
            ...
            for (int i = 0; i < this.modules.size(); ++i) {
                TabGUI.this.fontRenderer.drawString(upperCase ? this.modules.get(i).getName().toUpperCase() : this.modules.get(i).getName(), (float)(x + 2), (float)(y + TabGUI.this.tabHeight.asInteger() * i + TabGUI.this.textPositionY.asInteger()), this.modules.get(i).getState() ? 16777215 : new Color(205, 205, 205).getRGB(), TabGUI.this.textShadow.asBoolean());
            }
        }
    }
