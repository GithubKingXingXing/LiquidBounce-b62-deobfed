...
public class ItemESP extends Module
{
                ...
                if (args.length > 1 && args[1].equalsIgnoreCase("color")) {
                    if (args.length > 2) {
                        if (args[2].equalsIgnoreCase("rainbow")) {
                            ...
                            ItemESP$1.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                            return;
                        }
                        if (args.length > 4) {
                            try {
    ...
    public void onRender3D(final Render3DEvent event) {
        if (this.modeValue.asString().equalsIgnoreCase("Box")) {
            ...
            ItemESP.mc.theWorld.loadedEntityList.stream().filter(entity -> entity instanceof EntityItem || entity instanceof EntityArrow).forEach(entity -> RenderUtils.drawEntityBox(entity, color, true));
        }
    }
    ...
    public void onRender2D(final Render2DEvent event) {
        if (this.modeValue.asString().equalsIgnoreCase("ShaderOutline")) {
            ...
            ItemESP.mc.theWorld.loadedEntityList.stream().filter(entity -> entity instanceof EntityItem || entity instanceof EntityArrow).forEach(entity -> ItemESP.mc.getRenderManager().renderEntityStatic(entity, event.getPartialTicks(), true));
