...
public class LiquidFontRenderer extends FontRenderer
{
    ...
    public void drawCenteredString(final String s, final int x, final int y, final int color, final boolean shadow) {
        this.drawString(s, (float)(x - this.getStringWidth(s) / 2), (float)y, color, shadow);
    }
    ...
    public int drawString(String text, final float x, float y, final int color, final boolean shadow) {
            ...
            for (int index = 0; index < parts.length; ++index) {
                if (parts[index].length() > 0) {
                    if (index == 0) {
                        ...
                        final float u = this.FONT_HEIGHT / 16.0f;
    ...
    public int getCharWidth(final char character) {
        return this.getStringWidth(Character.toString(character));
    }
