...
public abstract class Panel
{
    ...
    public void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) {
        ...
        if (mouseButton == 1 && this.isHovering(mouseX, mouseY)) {
            this.open = !this.open;
            Minecraft.getMinecraft().getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.bow"), 1.0f));
            return;
        }
        for (final Element element : this.elements) {
    ...
    boolean isHovering(final int mouseX, final int mouseY) {
        final float textWidth = Minecraft.getMinecraft().fontRendererObj.getStringWidth(StringUtils.stripControlCodes(this.name)) - 100.0f;
        return mouseX >= this.x - textWidth / 2.0f - 19.0f && mouseX <= this.x - textWidth / 2.0f + Minecraft.getMinecraft().fontRendererObj.getStringWidth(StringUtils.stripControlCodes(this.name)) + 19.0f && mouseY >= this.y && mouseY <= this.y + this.height - (this.open ? 2 : 0);
    }
}
