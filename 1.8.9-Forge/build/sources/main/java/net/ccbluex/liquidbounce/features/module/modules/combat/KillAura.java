...
public class KillAura extends Module
{
                    ...
                    if (args[1].equalsIgnoreCase("maxcps")) {
                        if (args.length > 2) {
                            try {
                                ...
                                KillAura$7.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                            }
    ...
    public void onMotion(final MotionEvent event) {
        if (KillAura.mc.thePlayer == null || this.target == null || this.currentTarget == null || event.getEventState() == EventState.POST || KillAura.mc.thePlayer.isSpectator() || !this.isLiving((EntityLivingBase)KillAura.mc.thePlayer) || ModuleManager.getModule(Blink.class).getState() || ModuleManager.getModule(FreeCam.class).getState()) {
            return;
        }
        AxisAlignedBB axisAlignedBB = this.target.getEntityBoundingBox();
        if (this.predictValue.asBoolean()) {
            axisAlignedBB = axisAlignedBB.offset((this.target.posX - this.target.prevPosX) * RandomUtils.nextFloat(this.minPredictSize.asFloat(), this.maxPredictSize.asFloat()), (this.target.posY - this.target.prevPosY) * RandomUtils.nextFloat(this.minPredictSize.asFloat(), this.maxPredictSize.asFloat()), (this.target.posZ - this.target.prevPosZ) * RandomUtils.nextFloat(this.minPredictSize.asFloat(), this.maxPredictSize.asFloat()));
        }
        ...
        if (this.silentRotationValue.asBoolean()) {
            ...
            KillAura.mc.thePlayer.rotationYaw = limitRotations[0];
            KillAura.mc.thePlayer.rotationPitch = limitRotations[1];
        }
        if (this.attackTimer.hasTimePassed(this.attackDelay) && this.currentTarget.hurtTime <= this.hurtTimeValue.asInteger()) {
            ...
            final boolean openInventory = this.aacValue.asBoolean() && KillAura.mc.currentScreen instanceof GuiInventory;
            final boolean failHit = failRate > 0.0f && RandomUtils.getRandom().nextInt(100) <= failRate;
            if (openInventory) {
                KillAura.mc.getNetHandler().addToSendQueue((Packet)new C0DPacketCloseWindow());
            }
            if (!this.hitable || failHit) {
                ...
                if (!multi) {
                    ...
                    if (openInventory) {
                        KillAura.mc.getNetHandler().addToSendQueue((Packet)new C16PacketClientStatus(C16PacketClientStatus.EnumState.OPEN_INVENTORY_ACHIEVEMENT));
                    }
                    ...
                    for (final Entity entity : KillAura.mc.theWorld.loadedEntityList) {
                        if (this.isSelected(entity) && (!this.multiModeValue.asString().equalsIgnoreCase("NCP") || RotationUtils.getRotationDifference(entity) <= 40.0) && KillAura.mc.thePlayer.getDistanceToEntity(entity) <= this.getRange(entity)) {
                ...
                this.prevTargetEntities.add(this.aacValue.asBoolean() ? this.target.getEntityId() : this.currentTarget.getEntityId());
    ...
    public void onRender3D(final Render3DEvent event) {
        if (KillAura.mc.thePlayer == null || KillAura.mc.thePlayer.isSpectator() || !this.isLiving((EntityLivingBase)KillAura.mc.thePlayer) || ModuleManager.getModule(Blink.class).getState() || ModuleManager.getModule(FreeCam.class).getState()) {
            this.target = null;
            this.currentTarget = null;
            this.hitable = false;
            this.stopBlocking();
        ...
        if (this.noInventoryAttackValue.asBoolean() && (KillAura.mc.currentScreen instanceof GuiContainer || System.currentTimeMillis() - this.containerOpen < this.noInventoryDelayValue.asInteger())) {
            this.target = null;
            this.currentTarget = null;
            this.hitable = false;
        ...
        if (this.raycastValue.asBoolean()) {
            final Entity raycastedEntity = RaycastUtils.raycastEntity(this.rangeValue.asFloat() - (KillAura.mc.thePlayer.isSprinting() ? this.rangeSprintReducementValue.asFloat() : 0.0f), entity -> (!this.livingRaycastValue.asBoolean() || (entity instanceof EntityLivingBase && !(entity instanceof EntityArmorStand))) && (this.isSelected(entity) || this.raycastIgnoredValue.asBoolean() || (this.aacValue.asBoolean() && !KillAura.mc.theWorld.getEntitiesWithinAABBExcludingEntity(entity, entity.getEntityBoundingBox()).isEmpty())));
    ...
    private void updateTarget() {
        ...
        for (final Entity entity2 : KillAura.mc.theWorld.loadedEntityList) {
            if (this.isSelected(entity2) && KillAura.mc.thePlayer.getDistanceToEntity(entity2) <= this.getRange(entity2) && RotationUtils.getRotationDifference(entity2) <= (float)this.fovValue.asObject() && ((EntityLivingBase)entity2).hurtTime <= (int)this.hurtTimeValue.asObject()) {
        ...
        switch (lowerCase) {
            case "distance": {
                targets.sort(Comparator.comparingDouble(value -> KillAura.mc.thePlayer.getDistanceToEntity(value)));
                break;
            }
            case "health": {
    ...
    private void showCriticals(final EntityLivingBase target) {
        ...
        for (int i = 0; i < 3; ++i) {
            if ((KillAura.mc.thePlayer.fallDistance > 0.0f && !KillAura.mc.thePlayer.onGround && !KillAura.mc.thePlayer.isOnLadder() && !KillAura.mc.thePlayer.isInWater() && !KillAura.mc.thePlayer.isPotionActive(Potion.blindness) && KillAura.mc.thePlayer.ridingEntity == null) || (criticals.getState() && criticals.msTimer.hasTimePassed(criticals.delayValue.asInteger()) && !KillAura.mc.thePlayer.isInWater() && !KillAura.mc.thePlayer.isInLava() && !KillAura.mc.thePlayer.isInWeb)) {
                KillAura.mc.thePlayer.onCriticalHit((Entity)target);
            }
            if (EnchantmentHelper.getModifierForCreature(KillAura.mc.thePlayer.getHeldItem(), target.getCreatureAttribute()) > 0.0f || this.fakeSharpValue.asBoolean()) {
                KillAura.mc.thePlayer.onEnchantmentCritical((Entity)target);
            }
        }
    }
    
    private boolean isSelected(final Entity entity) {
        if (!(entity instanceof EntityLivingBase) || (!EntityUtils.targetDead && !this.isLiving((EntityLivingBase)entity)) || entity.equals((Object)KillAura.mc.thePlayer) || (!EntityUtils.targetInvisible && entity.isInvisible())) {
            return false;
        }
    ...
    public void attackEntity(final EntityLivingBase targetEntity) {
        if (KillAura.mc.thePlayer.isBlocking() || this.blocking) {
        ...
        KillAura.mc.getNetHandler().addToSendQueue((Packet)new C02PacketUseEntity((Entity)targetEntity, C02PacketUseEntity.Action.ATTACK));
        if (this.keepSprintValue.asBoolean()) {
            if (KillAura.mc.thePlayer.fallDistance > 0.0f && !KillAura.mc.thePlayer.onGround && !KillAura.mc.thePlayer.isOnLadder() && !KillAura.mc.thePlayer.isInWater() && !KillAura.mc.thePlayer.isPotionActive(Potion.blindness) && KillAura.mc.thePlayer.ridingEntity == null) {
                KillAura.mc.thePlayer.onCriticalHit((Entity)targetEntity);
            }
            if (EnchantmentHelper.getModifierForCreature(KillAura.mc.thePlayer.getHeldItem(), targetEntity.getCreatureAttribute()) > 0.0f) {
                KillAura.mc.thePlayer.onEnchantmentCritical((Entity)targetEntity);
            }
        }
        else if (KillAura.mc.playerController.getCurrentGameType() != WorldSettings.GameType.SPECTATOR) {
            KillAura.mc.thePlayer.attackTargetEntityWithCurrentItem((Entity)targetEntity);
        }
        if (KillAura.mc.thePlayer.isBlocking() || (this.autoBlockValue.asBoolean() && KillAura.mc.thePlayer.getHeldItem() != null && KillAura.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)) {
            ...
            if (this.interactAutoBlockValue.asBoolean()) {
                KillAura.mc.getNetHandler().addToSendQueue((Packet)new C02PacketUseEntity((Entity)targetEntity, targetEntity.getPositionVector()));
                KillAura.mc.getNetHandler().addToSendQueue((Packet)new C02PacketUseEntity((Entity)targetEntity, C02PacketUseEntity.Action.INTERACT));
            }
    ...
    private boolean isLiving(final EntityLivingBase entity) {
        return (entity.isEntityAlive() && entity.getHealth() > 0.0f) || (this.aacValue.asBoolean() && entity.hurtTime > 5);
    }
    
    private float getRange(final Entity entity) {
        return (KillAura.mc.thePlayer.canEntityBeSeen(entity) ? this.rangeValue.asFloat() : this.throughWallsRangeValue.asFloat()) - (KillAura.mc.thePlayer.isSprinting() ? this.rangeSprintReducementValue.asFloat() : 0.0f);
    }
