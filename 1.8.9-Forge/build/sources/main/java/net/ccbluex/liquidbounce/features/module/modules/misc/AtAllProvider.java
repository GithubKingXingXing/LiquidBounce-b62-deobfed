                    ...
                    if (args[1].equalsIgnoreCase("maxdelay")) {
                        if (args.length > 2) {
                            try {
                                ...
                                AtAllProvider$3.mc.getSoundHandler().playSound((ISound)PositionedSoundRecord.create(new ResourceLocation("random.anvil_use"), 1.0f));
                            }
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        try {
            synchronized (this.sendQueue) {
                ...
                AtAllProvider.mc.thePlayer.sendChatMessage((String)this.sendQueue.take());
                this.msTimer.reset();
                this.delay = TimeUtils.randomDelay(this.minDelayValue.asInteger(), this.maxDelayValue.asInteger());
            }
        }
    ...
    public void onPacket(final PacketEvent event) {
        if (event.getPacket() instanceof C01PacketChatMessage) {
            ...
            if (message.contains("@a")) {
                synchronized (this.sendQueue) {
                    for (final NetworkPlayerInfo playerInfo : AtAllProvider.mc.getNetHandler().getPlayerInfoMap()) {
                        final String playerName = playerInfo.getGameProfile().getName();
                        if (playerName.equals(AtAllProvider.mc.thePlayer.getName())) {
                            continue;
                        }
