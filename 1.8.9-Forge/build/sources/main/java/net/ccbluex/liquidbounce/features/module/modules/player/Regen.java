...
public class Regen extends Module
{
    ...
    public void onUpdate(final UpdateEvent event) {
        ...
        if ((!this.noAirValue.asBoolean() || Regen.mc.thePlayer.onGround) && !Regen.mc.thePlayer.capabilities.isCreativeMode && Regen.mc.thePlayer.getFoodStats().getFoodLevel() > this.foodValue.asInteger() && Regen.mc.thePlayer.isEntityAlive() && Regen.mc.thePlayer.getHealth() < this.healthValue.asInteger()) {
            final String lowerCase = this.modeValue.asString().toLowerCase();
            switch (lowerCase) {
                case "vanilla": {
                    for (int i = 0; i < speed; ++i) {
                        Regen.mc.getNetHandler().addToSendQueue((Packet)new C03PacketPlayer());
                    }
                    break;
                }
                case "spartan": {
